...
function Server(options) {
  ...
  registry.on('deregister', function(location) {
    this.emit('deregister', location);
  });

  var router = Router({
    udpWindow: opts.udpWindow,
    routes: [
      [ 'POST',    /\/rd$/,    this._validate ],
...
Server.prototype._validate = function(req, res) {
  ...
  return new Promise(function(resolve, reject) {
    if (utils.validateQuery(query, ['ep'])) {
      this.emit('register', params, function(err) {
        if (err) {
          reject(err);
        } else {
          resolve();
...
Server.prototype._request = function(ep, params) {
  ...
  return registry.find(ep)
  .then(proxy)
  .then(utils.send.bind(null, type, request))
  });
}
